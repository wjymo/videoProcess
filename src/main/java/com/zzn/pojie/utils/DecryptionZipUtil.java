...
import com.zzn.pojie.zip.UnZipUtils;
...
//        unzip("D:\\ximo\\xxx.zip", "D:\\ximo\\xxx", "111");

        try (BufferedReader br = new BufferedReader(new FileReader("D:\\密码字典1.txt"));) {
            String str = null;
            List<String> pwdList=new ArrayList<>();
            while ((str = br.readLine()) != null) {
                pwdList.add(str);
                long sum = LONG_ADDER.sum();
                if((sum+1)%10000==0){
                    List<String> pwdList2=new ArrayList<>();
                    pwdList2.addAll(pwdList);
                    Task task=new Task(pwdList2);
                    ThreadExecutorUtils.POJIE_EXECUTOR.submit(task);
                    pwdList.clear();
                }
    ...
    public static  class Task implements Callable<String>{
        List<String> pwdList;
        public Task(List<String> pwdList) {
...
//                    boolean unzip = unzip("D:\\ximo\\李熙墨视频.zip", "D:\\ximo\\shiping", pwd);
                    boolean unzip = UnZipUtils.unZip("D:\\ximo\\李熙墨《100天》完结.zip", "D:\\ximo\\shiping", pwd);
                    if(unzip){
                        System.out.println("解开压缩包密码，密码为："+pwd);
    ...
    private static long readByChannel() {
        long counts = 0;
        File file = new File("D:\\密码字典1.txt");
    ...
    private static long readByBos(File file) {
        long counts = 0;
        int offset = 0;
